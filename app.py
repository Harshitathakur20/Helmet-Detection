...
# Screenshot save folder - auto created
SCREENSHOT_DIR = pathlib.Path(r"C:\\Users\\welcome\\Downloads\\actual project\\actual project\\violations")
...
with st.sidebar:
    ...
    if st.button("↺  Reset Session"):
        for k, v in defaults.items():
            if k != "session_start":
                st.session_state[k] = set() if isinstance(v, set) else v
...
def save_violation_screenshot(frame, head_box):
    """
    Save ONE full-frame screenshot per unique head position.
    Uses a 10x10 grid to track if this person was already photographed.
    Screenshot is saved with red box + timestamp to the violations/ folder.
    """
    x1, y1, x2, y2 = head_box
    h, w = frame.shape[:2]

    # Grid cell key — divide frame into 10x10 zones
    col = int(((x1 + x2) / 2) / (w / 10))
    row = int(((y1 + y2) / 2) / (h / 10))
    key = (col, row)

    if key in st.session_state["captured_heads"]:
        return  # already photographed this person

    st.session_state["captured_heads"].add(key)
    st.session_state["screenshot_count"] += 1

    # Draw red box + timestamp on a copy of the full frame
    shot = frame.copy()
    cv2.rectangle(shot, (x1, y1), (x2, y2), (0, 0, 255), 3)
    ts = datetime.now().strftime("%Y-%m-%d  %H:%M:%S")
    label = f"VIOLATION - NO HELMET  |  {ts}"
    cv2.rectangle(shot, (0, h - 40), (w, h), (0, 0, 0), -1)
    cv2.putText(shot, label, (10, h - 12),
                cv2.FONT_HERSHEY_SIMPLEX, 0.65, (0, 0, 255), 2)

    # Save to violations/ folder
    fname = SCREENSHOT_DIR / f"violation_{datetime.now().strftime('%Y%m%d_%H%M%S_%f')}.jpg"
    cv2.imwrite(str(fname), shot, [cv2.IMWRITE_JPEG_QUALITY, 95])
    add_log("violation", f"Screenshot saved: {fname.name}")
...
def process_frame(frame):
    ...
    # Count violations AND save screenshot for each violating head
    violations = 0
    fh, fw = frame.shape[:2]
    for h in heads:
        if not any(iou(h, p) > iou_thresh for p in helmets):
            violations += 1
            save_violation_screenshot(frame, h)
...
def render_stats_panel(stat_slots, violations, heads, helmets):
    ...
    s4.markdown(f'<div class="sc amber"><div class="sc-num">{st.session_state["screenshot_count"]}</div><div class="sc-label">📸 Saved</div></div>', unsafe_allow_html=True)
